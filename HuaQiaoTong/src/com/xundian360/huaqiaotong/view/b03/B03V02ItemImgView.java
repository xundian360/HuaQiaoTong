...
public class B03V02ItemImgView {
	...
	private void initData() {
		...
		operationSelect = new SimpleListDialog(context, onselect, tittleName, listData);
	...
	private void initView() {
		mainView = LayoutInflater.from(context).inflate(R.layout.b03v02_item_img, null);
		...
		itemTittle.setText(imgPath.substring(imgPath.lastIndexOf(File.separatorChar)));
	...
	private void setPic() {
		...
		Bitmap imgBitmap = PhotoSelectUtil.changePostItemImgSize(context, source);
		
		// 存储图片到SD卡
		String savePath = context.getCacheDir().getAbsolutePath() + File.separatorChar  + PhotoSelectUtil.getPhotoFileName();
