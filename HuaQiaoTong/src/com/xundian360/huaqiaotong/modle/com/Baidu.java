 ...
 * KTV对象
 * 
 * @description 百度数据 { status : 0, message : "ok", total : 1602, results : [ {
 *              name : "钱柜(惠新旗舰店)", location : { lat : 39.993388, lng :
 *              116.430796 }, address : "惠新东街乙4号", telephone : "(010)59639666",
 *              uid : "87248d96c43561aa2806ce53", detail_info : { type : "life",
 *              tag : "KTV,对外经贸", detail_url :
 *              "http://api.map.baidu.com/place/detail?uid=87248d96c43561aa2806ce53&output=html&source=placeapi_v2"
 *              , price : "88", overall_rating : "4.0", image_num : "278",
 *              groupon_num : 3, comment_num : "1795" } } ] }
 * 
 * @author TengTeng
 * @date 2013-10-4
 * @version 1.0
...
public class Baidu implements Serializable, Comparable<Baidu> {
	...
	public Baidu(String name, double location_lat, double location_lng,
			...
			String shop_pic_soulue, String image_num, String comment_num) {
		super();
		this.name = name;
	...
	public void setRelaDistance(double lan, double lon) {
		// 计算经纬度的距离
		this.relaDistance = CommonUtil.distanceOfTwoPoints(location_lng,
				location_lat, lon, lan, GaussSphere.Beijing54);
	}
	...
	@Override
	public Baidu clone() {
		return new Baidu(this.name, this.location_lat, this.location_lng,
				this.address, this.telephone, this.uid, this.distance,
				this.detail_url, this.price, this.overall_rating,
				this.image_num, this.shop_pic_soulue, this.comment_num);
	}
	...
	public int compareTo(Baidu another) {
		if (this.relaDistance != another.getRelaDistance()) {
			return (int) Math.ceil(this.relaDistance
					- another.getRelaDistance());
		}
		return 0;
	}
