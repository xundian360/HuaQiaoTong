...
public class B02V00Activity extends ComNoTittleMapActivity {
	...
	private void initData() {
		...
		// 设置数据源
		zoonAdapter = new SimpleAdapter(this, data, R.layout.b02v00_zoon_item,
				from, to);
	...
	private void setAdapterDate() {
		...
		idZoon = getResources().getStringArray(
				R.array.b02v00_group_0_items_code);
	...
	public void getCarListView(final int zoonIndex) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// 取得出租车信息
				final List<Texi> taxis = B02BaiduUtil.getCarsList(
						B02V00Activity.this, tittleId);
	...
	Runnable updateMap = new Runnable() {
		...
		public void run() {
			...
			if (carPiontList != null && !carPiontList.isEmpty()) {
				...
				// 气泡列表
				CarsPointItemOverlay mOverlay = new CarsPointItemOverlay(
						B02V00Activity.this, getResources().getDrawable(
								R.drawable.icon_gcoding), mapView, carPiontList);
