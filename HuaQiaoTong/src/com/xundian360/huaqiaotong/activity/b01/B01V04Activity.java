...
public class B01V04Activity extends ComNoTittleMapActivity {
	...
	private void initData() {
		...
		itemObject = (ItemObject) getIntent().getSerializableExtra(
				B01V00Activity.ITEM_OBJECT_KEY);
	...
	public void myLocationOtherDo(BDLocation location) {
		...
		mapView.getController().animateTo(
				new GeoPoint((int) (itemsData.get(0).getLocation_lat() * 1e6),
						(int) (itemsData.get(0).getLocation_lng() * 1e6)));
	};
	...
	private void initOverlay() {
		...
		if (itemsData != null && !itemsData.isEmpty()) {
			...
			BaiduItemOverlay mOverlay = new BaiduItemOverlay(this,
			...
			for (Baidu ktv : itemsData) {
				...
				GeoPoint geoPoint = new GeoPoint(
						(int) (ktv.getLocation_lat() * 1e6),
						(int) (ktv.getLocation_lng() * 1e6));

				Log.d("debug",
						"station.getLocation_lat() > "
								+ (int) (ktv.getLocation_lat() * 1e6));
				Log.d("debug",
						"station.getLocation_lng() > "
								+ (int) (ktv.getLocation_lng() * 1e6));
