...
public class B00V05Activity extends ComNoTittleBMapManActivity {
	...
	private void initData() {
		...
		// 取得传递来的参数
		startStation = (Baidu) getIntent().getSerializableExtra(
				START_STATION_KEY);
		endStation = (Baidu) getIntent().getSerializableExtra(END_STATION_KEY);

		searchWay = getIntent().getIntExtra(SEARCH_WAY_KEY,
				B00v00HuanchengView.TRANSITSEARCH_KEY);

		// 设置adapter
		adapter = new B00V05PlanAdapter(this, data, R.layout.b00v06_item, from,
				to);
	}
	...
	private void getPlanData() {
		...
		if (searchWay == B00v00HuanchengView.DRIVINGSEARCH_KEY) {
			mSearch.drivingSearch(getString(R.string.common_city), startNote,
					getString(R.string.common_city), endNote);
		}

		// 步行
		else if (searchWay == B00v00HuanchengView.WALKINGSEARCH_KEY) {
			mSearch.walkingSearch(getString(R.string.common_city), startNote,
					getString(R.string.common_city), endNote);
		}

		// 检索公交
		else {
			mSearch.transitSearch(getString(R.string.common_city), startNote,
					endNote);
		}
	}
	...
	private void setAdapterData() {
		...
		for (int i = 0; i < huanchengs.getNumPlan(); i++) {
			...
			String fanganTittle = getString(R.string.b00v05_fangan_text,
					(i + 1) + "");
		...
		// 创建方案选择Dialog
		fanganListDialog = new SimpleListDialog(this, onPlanSelect,
				getString(R.string.b00v05_other_fangan_text), fanganListData);
	}
	...
	OnClickListener otherFanganClick = new OnClickListener() {
		...
		public void onClick(View arg0) {
			...
			if (fanganListData == null && fanganListData.length == 1) {
				ShowMessageUtils.show(B00V05Activity.this,
						R.string.b00v05_no_other_fangan_text);
			}
	...
	MySearchListener hCSearchListener = new MySearchListener() {
		...
		public void onGetTransitRouteResult(MKTransitRouteResult result,
			...
			if (iError == MKEvent.ERROR_RESULT_NOT_FOUND) {
				Toast.makeText(B00V05Activity.this, "抱歉，未找到结果",
						Toast.LENGTH_LONG).show();
				return;
			} else if (iError != 0 || result == null) {
				Toast.makeText(B00V05Activity.this, "搜索出错啦..",
						Toast.LENGTH_LONG).show();
				return;
			}
			...
			// 设置标题
			tittleText.setText(getString(R.string.b00v05_tittle_text,
					result.getNumPlan() + ""));
