...
public class BusOperatingHelper extends BusStationOperatingHelper {
	...
	public List<Bus> getAllBuses() {
		...
		// 取得查找Cursor
		Cursor cursor = selectRecode(TABLE_BUS_ROUTE, null, null, null, null,
				null, null);
	...
	public List<Bus> getStationBus(Station station) {
		...
		// 根据ID取得车辆信息
		List<Bus> buses = getBusByBusId((String[]) busesIds
				.toArray(new String[busesIds.size()]));

		return buses;
	}

	/**
	 * 取得Station对应的Bus信息 不包含参数指定的信息
	 * 
	 * @param bus
	 */
	public List<Bus> getStationBusNotInList(Station station, List<Bus> buses) {
		...
		// 遍历
		for (Bus bus : buses) {
			sql_condition.append("'").append(bus.getRouteId()).append("',");
		}

		sql_condition = sql_condition.replace(sql_condition.length() - 1,
				sql_condition.length(), "");

		sql_condition.append(")").append(" and busStopId=?");
		...
		// 根据ID取得车辆信息
		List<Bus> buses2 = getBusByBusId((String[]) routeIds
				.toArray(new String[routeIds.size()]));
