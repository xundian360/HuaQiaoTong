...
public class B02BaiduUtil extends BaiduUtil {
	 ...
	 * @throws JSONException
	 */
	private static SerializableList getCarsPointFromJson(String json)
			throws JSONException {
		...
		// 当查询成功，并且结果集部为空的时候
		if (STATUS_OK_KEY.equals(status) && carsPointResults != null
			...
			for (int i = 0; i < carsPointResults.length(); i++) {
				...
				if (locationJ != null) {
					baiduItem.setLocation_lat(StringUtils.paseDouble(locationJ
							.get(1).toString(), 0));
					baiduItem.setLocation_lng(StringUtils.paseDouble(locationJ
							.get(0).toString(), 0));
				}
	...
	private static List<Texi> getCarsFromJson(String json) throws JSONException {
		...
		// 当查询成功，并且结果集部为空的时候
		if (STATUS_OK_KEY.equals(status) && carsResults != null
