...
public class B00v00BusUtil {
	 ...
	 */
	public static List<NetStation> getInfoOfStation(Context context,
			String busStationId) throws JSONException {

		List<NetStation> buses = new ArrayList<NetStation>();

		// 网络上加载车辆信息
		String infoOfStationJson = getInfoOfStationFromNet(context,
				busStationId);
	...
	private static String getInfoOfStationFromNet(Context context,
			String busStationId) {
		...
		try {
			// 访问网络
			infoOfStationJson = BaseHttpClient.doGetRequest(
					context.getString(R.string.comm_get_bus_station), params);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @throws JSONException
	 */
	private static void setBusFromJson(List<NetStation> buses, String json)
			throws JSONException {
		...
		if (busesArr != null) {
			...
			for (int i = 0; i < busesArr.length(); i++) {
				...
				// 取得经过的车辆信息
				List<NetStationItem> stationItems = getStationItems(busJ
						.getString("stationInfoNoList"));
	 ...
	 * @throws JSONException
	 */
	private static List<NetStationItem> getStationItems(String jsonItem)
			throws JSONException {
		...
		if (busItemArr != null && busItemArr.length() > 0) {
			...
			for (int i = 0; i < busItemArr.length(); i++) {
				...
				stationItem
						.setItemIndex(netStationItemJ.getString("itemIndex"));
				...
				stationItem
						.setStopCount(netStationItemJ.getString("stopCount"));
	 ...
	 * @throws JSONException
	 */
	public static List<NetLine> getInfoOfRoute(Context context,
			String busRouteId) throws JSONException {
	 ...
	 */
	@SuppressWarnings("finally")
	private static String getInfoOfRouteFromNet(Context context,
			String busRouteId) {
		...
		try {
			// 访问网络
			infoOfStationJson = BaseHttpClient
					.doGetRequest(
							context.getString(R.string.comm_get_route_bus_info),
							params);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @throws JSONException
	 */
	private static List<NetLine> setRouteFromJson(String json)
			throws JSONException {
	 ...
	 * @throws JSONException
	 */
	public static List<Huancheng> getInfoOfHuancheng(Context context,
			String busStopIdFrom, String busStopIdTo) throws JSONException {

		List<Huancheng> huanchengs = new ArrayList<Huancheng>();

		// 网络上加载站点信息
		String infoOfHuanchengJson = getInfoOfHuanchengFromNet(context,
				busStopIdFrom, busStopIdTo);
	 ...
	 */
	@SuppressWarnings("finally")
	private static String getInfoOfHuanchengFromNet(Context context,
			String busStopIdFrom, String busStopIdTo) {
		...
		try {
			// 访问网络
			infoOfStationJson = BaseHttpClient.doGetRequest(
					context.getString(R.string.comm_get_change_bus_info),
					params);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @throws JSONException
	 */
	private static List<Huancheng> setHuanhchengFromJson(String json)
			throws JSONException {
