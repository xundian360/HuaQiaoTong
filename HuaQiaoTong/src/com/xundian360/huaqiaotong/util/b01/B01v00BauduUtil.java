...
public class B01v00BauduUtil extends BaiduUtil{
	 ...
	 * 城市内检索
	 * @param context
	 * @param pageSize     返回记录数量，默认为10条记录，最大返回结果为20条
	 * @param pageNum   分页页码，默认为0,0代表第一页，1代表第二页，以此类推。
	 * @param region         检索区域，如果取值为“全国”或某省份，则返回指定区域的POI
	 * @return
	 */
	public static Map<String, Object> searchRegionKtv(Context context, String pageSize, String pageNum, String region){
	 ...
	 * @param pageSize     返回记录数量，默认为10条记录，最大返回结果为20条
	 * @param pageNum   分页页码，默认为0,0代表第一页，1代表第二页，以此类推。
	 * @param bounds        检索矩形区域（39.915,116.404,39.975,116.414）
	 * @return
	 */
	public static Map<String, Object> searchBoundsKtv(Context context, String query, String pageSize, String pageNum, String bounds){
	 ...
	 * @param pageSize     返回记录数量，默认为10条记录，最大返回结果为20条
	 * @param pageNum   分页页码，默认为0,0代表第一页，1代表第二页，以此类推。
	 * @param location       周边检索中心点（39.915,116.404）
	 * @param radius          周边检索半径，单位为米
	 * @return
	 */
	public static Map<String, Object> searchRadiusKtv(Context context, String query, String pageSize, String pageNum, 
	...
	private static Map<String, Object> searchKtv(Context context, String url, HashMap<String, String> parameter) {
	 ...
	 */
	private static Map<String, Object> getKtvFromJson(String ktvJsonString) throws JSONException {
		...
		if(ktvResults != null) {
			...
			for (int i = 0; i < ktvResults.length(); i++) {
				...
				if(locationJ != null) {
					ktv.setLocation_lat(StringUtils.paseDouble(JsonUtil.getString(locationJ, "lat"), 0));
					ktv.setLocation_lng(StringUtils.paseDouble(JsonUtil.getString(locationJ, "lng"), 0));
				}
				...
				if(detail != null) {
					...
					ktv.setOverall_rating(JsonUtil.getString(detail, "overall_rating"));
					ktv.setComment_num(JsonUtil.getString(detail, "comment_num"));
